...
const ChartContainer = React.forwardRef<
  ...
  React.ComponentProps<"div"> & {
    config: ChartConfig;
    children: any; // Bypassing strict ResponsiveContainer children typing
  }
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(config).filter(([_, conf]) => (conf as any).theme || (conf as any).color);
          ...
          .map(([theme, prefix]) => `
            ${prefix} [data-chart=${id}] {
              ${colorConfig.map(([key, itemConfig]) => {
                const color = (itemConfig as any).theme?.[theme as keyof typeof THEMES] || (itemConfig as any).color;
...
// Using 'any' for Tooltip props to stop the Payload/Label errors
      ...
      <div ref={ref} className={cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
        ...
        <div className="grid gap-1.5">
          {payload.map((item: any, index: number) => {
            ...
            return (
              <div key={item.dataKey || index} className={cn("flex w-full flex-wrap items-stretch gap-2", indicator === "center" && "items-center")}>
                {formatter && item?.value !== undefined ? formatter(item.value, item.name, item, index, item.payload) : (
                  <>
                    {itemConfig?.icon ? <itemConfig.icon /> : !hideIndicator && (
                      <div className={cn("h-2.5 w-2.5 shrink-0 rounded-[2px]")} style={{ backgroundColor: indicatorColor }} />
                    )}
...
// Using 'any' here solves the 'verticalAlign' vs HTML 'div' conflict
